<template>
 ...
 <div class="q-pa-md" loading>
    ...
    margin-left: 10.5rem;">
      ...
      <q-card-section style="margin-top: -3rem;
...
<div class="q-pa-md" style="margin-top: -15px;" @click="hiddenDay=true">
    ...
    <div style="max-width: 90%;" v-for="(item,index) in list" :key="index">
        ...
        ">
          ...
          <q-item-section side>
            <q-item-label class="text-h6" style="color: #355B75;">-¥ {{ removeZero(item.PayMoney) }}</q-item-label>
          </q-item-section>
        </q-item>
    </div>
  <div class="low">
  ...
  <q-dialog v-model="screen" persistent>
    <q-card>
        ...
        <q-toolbar-title>
        <q-icon name="report_problem"  style="font-size: 1.5rem;"/> 提示</q-toolbar-title>
        </div>
        </q-card-section>
        <q-card-section class="text-h6">
         本功能暂时不开放,请去农行APP内进行线上充值
        </q-card-section>
