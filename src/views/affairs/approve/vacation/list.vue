<template>
 ...
 <div class="q-pa-md">
    ...
    ">
      <q-card-section>
        <div class="text-h6" style="margin-left:6rem; height: 2rem;">教师请假出差审批</div>
      </q-card-section>
      <q-tabs
 ...
 <div class="q-pa-md flex justify-center">
    ...
    <div style="max-width: 90%;"  v-for="(item,index) in list" :key="index">
        ...
        ">
          <q-item-section>
            <q-item-label class="text-h6" lines="1" v-if="item.attribute == 2">{{strTo(item.teacher)}} 出差 {{item.days}}  天</q-item-label>
            <q-item-label class="text-h6" lines="1" v-else>{{strTo(item.teacher)}} 请假 {{item.days}}  天</q-item-label>
  ...
  <q-dialog v-model="detailvw">
      <q-card style="width: 100%;">
        <q-card-section>
          <div class="text-h6">{{this.details.teacher.name}}-申请请假 {{this.details.days}} 天</div>
...
import { getList, ApproveAction} from '@/api/affairs/Vacation'
export default {
  ...
  methods:{
    ...
    approver(id,act){
      ApproveAction({id:id,act:act}).then(res => {
