  ...
  <base-content>
    ...
    box-shadow: 0px 0px 2rem 0rem #c0c6cf;">
          <div class="q-gutter-y-md column">
            <q-card-section>
              <div class="text-h6" style="margin-left: 8rem;height:10px;">上传作业</div>
            </q-card-section>
...
import { Add } from '@/api/student/CourseWork'
export default {
  ...
  methods:{
    Save() {
      this.loadings()
      if(!this.form.results.length){
        this.Notify('作业内容不能为空','orange')
        return false
      }
      Add(this.form).then(res => {
        if (res.data.code == 200) {
          this.$refs.child.submit(this.form.pid,'CourseWork');
          this.$q.notify({
