  ...
  <base-content>
    <div class="q-pa-md">
      <div class="row">
            <q-card-section>
              <q-btn-group outline>
                <q-btn outline color="primary" label="彤旭智慧校园" icon="note_add" @click="$router.push('/effort/add')" />
              </q-btn-group>
            </q-card-section>
    ...
    <q-list v-for="(item,index) in list" :key="index">
      <q-item >
        ...
        <q-item-section side >
          <q-item-label class="text-h6" @click="skip('/operation?id='+item.id)">操作</q-item-label>
        </q-item-section>
      </q-item>
...
import { List } from '@/api/student/CourseWork'
export default {
