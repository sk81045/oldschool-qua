...
import { getUserRouter,getUserRouter2 } from '../api/UserApi'
...
router.beforeEach(async (to, from, next) => {
  ...
  if (token) {
    ...
    if (store.getters.getRoutes.length) {
      ...
      // const userRouterRespouse2 = await getUserRouter2()
      // const MergeRouterRespouse = userRouterRespouse.data.concat(userRouterRespouse2.data.router)
      // console.log('merges router group',MergeRouterRespouse)
      // 将从后台获取的路由转换为 vue-router 可用形式
      const MergeRouterRespouse = await getUserRouter2()
      const userRouter = handleJsonRouterToAsyncRouter(MergeRouterRespouse.data.router)
      // 在路由的末尾处添加 404 界面
